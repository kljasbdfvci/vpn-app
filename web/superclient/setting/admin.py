...
class LanConfigAdminForm(forms.ModelForm):
    interface = forms.ChoiceField(choices=tuple((key, key) for key in get_eth_interfaces()))
...
class DhcpServerConfigAdminForm(forms.ModelForm):
    interface = forms.ChoiceField(choices=tuple((key, key) for key in get_interfaces()))
